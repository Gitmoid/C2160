...
int find_skeletons() {
    ...
    for (int i = 0; i <= atoms_count; i++) {
        if (strcmp(atoms[i].atom_name, " N  ") == 0) {
            residues[j].peptide_skeleton[count] = atoms[i].atom_num;
            count++;
        ...
        if (i == residues[j].last_atom) {
            residues[j].skeleton_count = count;
            count = 0;
            j++;
        }
    }
...
void print_skeletons() {
    int k = 0;
    k = find_skeletons();

    for (int i = 0; i <= k; i++) {
        printf("Residuum %d %s, pater: ", residues[i].residue_num, residues[i].residue_name);
        if (residues[i].skeleton_count > 0) {
            for (int j = 0; j < residues[i].skeleton_count; j++) {
                printf("%i ", residues[i].peptide_skeleton[j]);
            }
        }
        printf("\n");
    }
